classdef IndentIdentifier < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        DetectorTypeSwitch              matlab.ui.control.Switch
        DetectorTypeSwitchLabel         matlab.ui.control.Label
        ...
        AddIndentButton_2               matlab.ui.control.Button
        IndentnotesTextArea             matlab.ui.control.TextArea
        IndentnotesLabel                matlab.ui.control.Label
    ...
    properties (Access = private)
        ...
        dataFolderName = 'Data';
        knoopCascadeDetector = 'KnoopCascadeDetector.xml';
        vickersCascadeDetector = 'VickersCascadeDetector.xml';
        knoopYOLODetector = 'KnoopYOLODetector.mat';
        vickersYOLODetector = 'VickersYOLODetector.mat';
    end
    ...
    methods (Access = private)
        ...
        function FindIndents(app, angleLessThan, angleGreaterThan)
            ...
            VickersYOLODetector = load(app.vickersYOLODetector);
            VickersYOLODetector = VickersYOLODetector.YOLOdetector;
            KnoopYOLODetector = load(app.knoopYOLODetector);
            KnoopYOLODetector = KnoopYOLODetector.YOLOdetector;
                
            numLines = 4;
                
            if strcmp(app.BoundingBoxesSwitch.Value, 'Automatic')
                if strcmp(app.DetectorTypeSwitch.Value, 'Cascade')
                    if strcmp(app.IndentTypeSwitch.Value, 'Knoop')
                        detector = vision.CascadeObjectDetector(app.knoopCascadeDetector);%initialize machine vision
                    else %If Vickers
                        detector = vision.CascadeObjectDetector(app.vickersCascadeDetector);%initialize machine vision
                    end 
                    foundBoundingBoxes = step(detector,app.originalImage);%detect all indents and put positions into matrix of boxes
                    release(detector);
                else %If YOLO detector
                    if strcmp(app.IndentTypeSwitch.Value, 'Knoop')
                        [foundBoundingBoxes, ~, ~] = detect(KnoopYOLODetector, app.originalImage);
                    else %If Vickers
                        [foundBoundingBoxes, ~, ~] = detect(VickersYOLODetector, app.originalImage);
                    end
                end 
        ...
        function showUI(app)
            ...
            app.DetectorTypeSwitch.Visible = 1;
            app.DetectorTypeSwitchLabel.Visible = 1;
        end
    end
    ...
    methods (Access = private)
        ...
        function CalculateButtonPushed(app, event)
            ...
            app.indentNotes = cell(length(app.indentImages));
        end

        % Button pushed function: CycleForward
        function CycleForwardButtonPushed(app, event)
            app.indentNotes{app.currentIndent} = app.IndentnotesTextArea.Value;
            app.currentIndent = app.currentIndent + 1;
            if app.currentIndent > length(app.indentImages)
                app.currentIndent = 1;
            end
            text = app.indentNotes{app.currentIndent};
            if ~isempty(text)
                app.IndentnotesTextArea.Value = text;
            else
                app.IndentnotesTextArea.Value = '';
            end
            updateScreens(app);
        end
        ...
        function BoundingBoxesSwitchValueChanged(app, event)
            ...
            if strcmp(value, 'Automatic')
                ...
                app.DetectorTypeSwitch.Visible = 0;
                app.DetectorTypeSwitchLabel.Visible = 0;
        ...
        function ExportButtonPushed(app, event)
            ...
            fstruct = dir(csvFile);
            if isempty(fstruct)%specific .csv file not found
                mat = ["Original Image", "Indent Image", "Indent Type", "Mass", "Micron/Pixel Ratio", "Skewed?", "HK (area)", "HK (diagonal)", "HV", "Notes"];
            ...
            for i = 1:1:length(app.boundingBoxes)
                ...
                indenterMass = app.MassgramsEditField.Value;
                micronPixelRatio = app.MicronPixelRatioEditField.Value;
                if ~isempty(app.indentNotes{i})
                    indentNotes = join(app.indentNotes{i});
                else
                    indentNotes = '';
                end
                if strcmp(app.IndentTypeSwitch.Value, "Knoop")
                ...
                M2(i,:) = [ogImage, indentImage, indentType, indenterMass, micronPixelRatio, skewed, HKa, HKd, HV, indentNotes];
            end
            writematrix(M2,'IndentData.csv','WriteMode','append')
            msgbox("Export complete! Path: " + which('IndentData.csv'));
        end

        % Button pushed function: AddIndentButton_2
        function AddIndentButton_2Pushed(app, event)
            app.DoneButton.Visible = 1;
            app.AddIndentButton_2.Visible = 0;
            app.ExportButton.Visible = 0;
            
            roi = drawrectangle(app.ImagePreview_2);
            app.boundingBoxes{end + 1} = roi.Position;
            addlistener(roi,'ROIMoved',@(varargin)allevents(app, roi));
        end
        ...
        function DoneButtonPushed(app, event)
            app.DoneButton.Visible = 0;
            app.AddIndentButton_2.Visible = 1;
            app.ExportButton.Visible = 1;
            
            app.indentNotes = [app.indentNotes(:); {''}];
            app.indentImages{end + 1} = imcrop(app.originalImage,app.boundingBoxes{end});
            app.originalIndentImages{end+1} =  app.indentImages{end};
            app.hardnessConstantsArea(end + 1) = 0;
            app.hardnessConstantsDiagonal(end + 1) = 0;
            app.newImage = insertShape(app.newImage, "rectangle", app.boundingBoxes{end}, "ShapeColor", "yellow", "LineWidth", 2);
            updateScreens(app);
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.BoundingBoxesSwitchLabel.Position = [463 298 100 22];
            ...
            app.BoundingBoxesSwitch.Position = [490 274 45 20];
            app.BoundingBoxesSwitch.Value = 'Automatic';
            ...
            app.MicronPixelRatioEditFieldLabel.Position = [417 171 102 22];
            ...
            app.MicronPixelRatioEditField.Position = [534 171 60 22];
            ...
            app.MassgramsEditFieldLabel.Position = [429 139 78 22];
            ...
            app.MassgramsEditField.Position = [522 139 60 22];
            ...
            app.IndentTypeSwitchLabel.Position = [475 358 71 22];
            ...
            app.IndentTypeSwitch.Position = [487 334 45 20];
            ...
            % Create DetectorTypeSwitchLabel
            app.DetectorTypeSwitchLabel = uilabel(app.SetupTab);
            app.DetectorTypeSwitchLabel.HorizontalAlignment = 'center';
            app.DetectorTypeSwitchLabel.FontWeight = 'bold';
            app.DetectorTypeSwitchLabel.Visible = 'off';
            app.DetectorTypeSwitchLabel.Position = [471 238 84 22];
            app.DetectorTypeSwitchLabel.Text = 'Detector Type';

            % Create DetectorTypeSwitch
            app.DetectorTypeSwitch = uiswitch(app.SetupTab, 'slider');
            app.DetectorTypeSwitch.Items = {'YOLO', 'Cascade'};
            app.DetectorTypeSwitch.Visible = 'off';
            app.DetectorTypeSwitch.Position = [490 214 45 20];
            app.DetectorTypeSwitch.Value = 'YOLO';
            ...
            app.IndentImage.Position = [270 139 334 196];
            ...
            app.ImagePreview_2.Position = [16 114 232 268];
            ...
            app.HKValueDiagonal.Position = [157 13 104 44];
            ...
            app.RecalculateButton.Position = [379 9 109 52];
            ...
            app.ConfirmButton.Position = [498 9 104 52];
            ...
            app.skewLabel.Position = [8 72 253 43];
            ...
            app.ExportButton.Position = [498 9 104 52];
            app.ExportButton.Text = 'Export';

            % Create IndentnotesLabel
            app.IndentnotesLabel = uilabel(app.ResultsTab);
            app.IndentnotesLabel.HorizontalAlignment = 'right';
            app.IndentnotesLabel.Position = [273 121 74 21];
            app.IndentnotesLabel.Text = 'Indent notes:';

            % Create IndentnotesTextArea
            app.IndentnotesTextArea = uitextarea(app.ResultsTab);
            app.IndentnotesTextArea.Placeholder = 'Type indent-specific notes here';
            app.IndentnotesTextArea.Position = [270 72 328 50];

            % Create AddIndentButton_2
            app.AddIndentButton_2 = uibutton(app.ResultsTab, 'push');
            app.AddIndentButton_2.ButtonPushedFcn = createCallbackFcn(app, @AddIndentButton_2Pushed, true);
            app.AddIndentButton_2.FontSize = 18;
            app.AddIndentButton_2.Position = [255 9 109 52];
            app.AddIndentButton_2.Text = 'Add Indent';

            % Create DoneButton
            app.DoneButton = uibutton(app.ResultsTab, 'push');
            ...
            app.DoneButton.Position = [498 9 100 52];
